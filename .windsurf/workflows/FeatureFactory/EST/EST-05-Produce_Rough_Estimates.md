...
**Order**: 5
**Phase**: Estimation
**Dependencies**: Predecessor: Activity 28 (Assess Risk Multipliers)
...
- Rough Estimates tab of `docs/plans/ESTIMATION_TEMPLATE.xlsx` (token budget — internal)
- Client Quote tab of `docs/plans/ESTIMATION_TEMPLATE.xlsx` (AFP + quote total — client-facing)
- Internal stakeholder summary (chat or `docs/plans/ESTIMATION_SUMMARY.md`)

## Artifacts Consumed

- Setup tab (K-token baselines, sprint parameters, internal combined multiplier)
- Scenario List tab (SP totals and FP totals per sprint)
- ECF + TCF tabs (internal combined multiplier)
- Client Quote tab (Stack Factor, Org Factor, $/FP from EST-04)
