...
**Order**: 6
**Phase**: Estimation
**Dependencies**: Predecessor: Activity 25 (Assess Estimation Readiness) — requires SAO.MD to exist

## Description

Produce commitment-quality estimates by running BPE-01 (Plan Feature) for each work package in estimation mode. BPE-01 already knows the architecture (SAO.md), the feature (scenario file), and the screen (dialogue map) — it can produce the exact artifact list. EST-06 wraps that with PERT triplets and a cross-check.
...
- WBS Features tab of `docs/plans/ESTIMATION_TEMPLATE.xlsx` (populated)
- Detailed Estimates tab of `docs/plans/ESTIMATION_TEMPLATE.xlsx` (populated)

## Artifacts Consumed

- `docs/architecture/SAO.md`
- `docs/features/` (BDD scenarios)
- `docs/architecture/screen-flow.drawio` (dialogue map — screen context for BPE-01)
- Setup tab (K-token baselines from Reference Table)
- Rough Estimates tab (L1 cross-check values)

## Notes

Level 2 is only worth doing when requirements are stable (≥80%) and SAO.md is current. If either condition fails, Level 1 estimates are good enough for the current sprint — schedule a Level 2 pass after the next rebaseline.
